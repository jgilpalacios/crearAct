class Calendario{
    ...
    constructor(anno, mesIn) {
		...
		for (let i=0;i<12;i++){
			...
			let aux=new Date(annEfec,pos,1);
			//alert(pos);
			let mes={
				anno:annEfec,
				numMes:pos+1,
				DiaSemanaInicial: aux.getDay(),
				nombre: this.NombreMes(pos+1),
				duracion: this.DiasMes(pos+1,annEfec),
				luna: this.Luna(pos+1,annEfec)
			};
			this.calen[i]=mes;
		}
	...
	EscribeCalen(sepMes,cabecera,cuerpo,dia){
		let html='';//alert(sepMes)
		this.calen.forEach((mes,i)=>{//alert(JSON.stringify(mes));
	...
	EscribeMesPos(pos,cabecera,cuerpo,dia){
		let mes=this.calen[pos];
		let html=cabecera.replace('#cab#', mes.nombre);//alert(html);
		let huecos=0;
		if(mes.DiaSemanaInicial===0) huecos=6
		else huecos=mes.DiaSemanaInicial-1;
		let thuecos='<tr>';
		let posd=0;
		...
		thuecos+='</tr>';//alert(thuecos);
		html+=cuerpo.replace('#cuerpo#',thuecos);
		return html;
	}

	EscribeMesPos2(pos,tratamiento,tnolec){//alert('ll');
		let mes=this.calen[pos];//alert('ll'+tratamiento)
		let html='<table style="width: 100%" border="1"><tbody>';
		let huecos=0;
		if(mes.DiaSemanaInicial===0) huecos=6
		else huecos=mes.DiaSemanaInicial-1;
		let thuecos='<tr>';
	...
	ponFestivo(dia,mes,atributo,valor){
		dFestivo[31*(mes-1)+dia][atributo]=valor||true;
	}
	...
	Luna(mes,anno){
		...
		let diasbis;
		if(anno<=1582){
			diasbis=Math.floor(annos/4)+13;//los 3 bisiestos seculares 1700,1800,1900 más los 10 dias suprimidos.
		}else diasbis=Math.floor(annos/4)-Math.floor(annos/100)+Math.floor(annos/400);
		let diasmes=0;//alert(mes);
		for(let i=0;i<mes-1;i++)diasmes+=this.DiasMes(i+1,anno)
		let dias=annos*365+diasbis+diasmes;
		//alert('dias: '+dias+' meseslun: '+parseInt(dias/29.530588)+' resto: '+(dias-parseInt(dias/29.530588)*29.530588));
		let luna=dias-parseInt(dias/29.530588)*29.530588-6;//posición en el mes lunar <=1 luna llena
		while(luna<0)luna=luna+29.530588;
		return luna;
	}

	DiasMes(mes,anno){//alert(mes)
		switch(mes-1){
			case 0:return 31;
			case 1:if(this.Bisiesto(anno)) return 29;
								else return 28;
			case 2:return 31;
			case 3:return 30;
			case 4:return 31;
