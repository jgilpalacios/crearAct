{
	var fecha=new Date();
	var dFestivo=new Array(372);//espacio para almacenar los festivos
	var calen;//array con todo el año
	...
	function ponFestivo(dia,mes){
		dFestivo[31*mes+dia]=true;
	}
	function Dia(dia,mes,anno,diaSem,festivo,luna){
		this.dia=dia;
		this.mes=mes;
		this.anno=anno;
		this.diaSem=diaSem;
	...
	function Luna(mes,anno){
		...
		diasmes=0;
		for(var i=0;i<mes;i++)diasmes+=DiasMes(i,anno)
	...
	function Mes(mes,anno){
		fecha=new Date(anno,mes,1);
		this.diaIn=fecha.getDay();
		if(anno<1582||(anno==1582&&mes<10)){
			...
			if((anno%100==0)&&(anno%400!=0)&&mes<2)this.diaIn-=1;//correccion del bisiesto en curso
	...
	function Festivo(dia,mes,anno){
		if(dFestivo[31*mes+dia]==true)return true;
		else return false;
	}
	function EscribeMes(mes, anno){
		//alert(mes);
		var mesaux=mes;
		mes=calen[mes];//new Mes(mes,anno);
	...
	function GeneraEditFEA(mmes,lfealongAnt){
		var tx="";
